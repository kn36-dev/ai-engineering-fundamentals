...
- **WAL (Write-Ahead Log)** — an append-only log of every change. Lives in `pg_wal/` as a sequence of 16MB segment files. Every modification is written here _before_ the corresponding shared buffers page can be considered modified.
...
3. **Backend modifies the page in shared buffers.** The page is now "dirty." Crucially, the change is _not yet_ in the data file on disk and _not yet_ in WAL.
