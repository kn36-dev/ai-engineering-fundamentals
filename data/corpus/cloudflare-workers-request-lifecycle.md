...
Workers run inside a single edge POP. Subrequests fan out from there. The DO hop is the only thing that _might_ cross POPs. Show the edge boundary as a wrapper group around the Worker, edge proxy, and any cached resources. KV/R2/D1 sit outside that group but still inside the Cloudflare boundary.
